...
sales %>%
    group_by(time_sale_month_of_year, time_sale_year, sv_is_outlier) %>%
  summarise(median_sale_price = median(meta_sale_price, na.rm = TRUE)) %>%
  ggplot() +
  geom_line(aes(x = time_sale_month_of_year, y = median_sale_price, color = sv_is_outlier)) +
  labs(y = "Median Sale Price", x = "Year", title = "By Year, Month, and Outlier Status") +
  theme_minimal() +
