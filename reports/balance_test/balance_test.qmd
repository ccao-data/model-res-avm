---
title: "Balance Testing Residential Parcels Across Sales"
execute:
  echo: false
  warning: false
format:
...
if (!file.exists(data_path)) {
  ...
  glue("
    ...
    SELECT
      vcri.*,
      CASE WHEN vps.pin IS NOT NULL THEN 1 ELSE 0 END AS sale,
    sale_date
    FROM model.vw_card_res_input vcri
    LEFT JOIN (
      SELECT pin, CAST(year(max(sale_date)) as varchar) as year, max(sale_date) AS sale_date
        FROM default.vw_pin_sale
        WHERE ((sale_price_log10
          BETWEEN sale_filter_lower_limit
          AND sale_filter_upper_limit)
          AND sale_filter_count >= 10)
        AND NOT is_multisale
        GROUP BY pin
    ) vps
      ON vcri.meta_pin = vps.pin and vcri.meta_year = vps.year
...
All covariates regressed on sale status - significance indicates a non-random effect on the likelihood of a sale ceterus paribus.  Sample is universe of '21 & '22 single family parcels.
...
temp <- ggplot(
  sales %>%
    filter(sv_is_outlier == "Not outlier" & meta_year >= '2019') %>%
  ...
  labs(y = "Median Sale Price", x = "Year",
       title = "Price / 2019 Price", subtitle = "By Tri, Township, and Year") +
  theme_minimal() +
  theme(
    legend.position = "none",
    legend.title = element_blank(),
...
sales %>%
  ...
  facet_wrap(vars(time_sale_year), scales = "free")
...
ggplot(
  ...
  geom_bar(aes(fill = sv_is_outlier)) +
  labs(y = "Number of Sales", x = "Year", title = "By Year, Month, and Outlier Status") +
  theme_minimal() +
...
ggplot(
  sales %>%
    group_by(meta_class) %>%
  summarise(percent = mean(case_when(sv_is_outlier == "Outlier" ~ 1, TRUE ~ 0))) %>%
    ungroup(),
