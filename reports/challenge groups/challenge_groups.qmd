...
cbind(results1, results2) %>%
  ...
  select(dataset, count, "median change" = median_change, "median ratio" = median_ratio, cod, prb, prd, mki, min, q25, median, "median sale" = median_sale, "median previous estimate" = median_prior_near_tot, q75, max, rmse, "r squared" = r_squared, mae, mpe, mape, mdape) %>%
  kable(format = "html", digits = 2, row.names = FALSE, format.args = list(big.mark = ",")) %>%
  kable_styling()
```
