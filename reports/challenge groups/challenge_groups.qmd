...
results_list <- lapply(names(datasets_full), function(name) {
  full_data_functions(datasets_full[[name]], name)
})
...
# Utilizes a list of functions in the evaluate stage to calculate stats for the list of Challenge Group Datasets.
...
results %>%
  as.data.frame() %>%
  mutate(
    median_change = ((median - median_prior_near_tot) / median_prior_near_tot) * 100,
    min = round(min),
    q25 = round(q25),
    median = round(median),
  ...
  select(dataset, count, "median yoy change" = median_change, "median ratio" = median_ratio, cod, prb, prd, mki, min, q25, "median predicted" = median, "median sale" = median_sale, "median previous estimate", q75, max) %>%
...
Sale and assessed values are capped at $2 million to increase legibility. 
