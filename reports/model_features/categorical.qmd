```{r}
source("_utils.R")
```

```{r}
#| cache: true
#| cache-extra: !expr rlang::hash(params)
#| cache-file-1: !expr rlang::hash_file("_baseline_query_data.R")
source("_baseline_query_data.R")
```


# Distributions of Features (Categorical)

::: {.panel-tabset}

### Assessment set

```{r feature_dist_assess_cat_transform}
new_cat_chars_assess_long <- new_assessment_data %>%
  select(all_of(categorical_preds)) %>%
  mutate(across(everything(), as.character)) %>%
  pivot_longer(
    cols = everything(),
    names_to = "predictor",
    values_to = "value"
  )
```

```{r feature_dist_assess_cat_plot}
#| column: screen-inset
#| fig-height: !expr fig_height(new_cat_chars_assess_long)

plot_small_multiple_histograms(new_cat_chars_assess_long, stat = "count")
```

```{r feature_dist_assess_cat_cleanup}
#| output: false

rm(new_cat_chars_assess_long)
gc()
```

### Training set

```{r feature_dist_train_cat_transform}
new_cat_chars_train_long <- new_training_data %>%
  select(all_of(categorical_preds)) %>%
  mutate(across(everything(), as.character)) %>%
  pivot_longer(
    cols = everything(),
    names_to = "predictor",
    values_to = "value"
  )
```

```{r feature_dist_train_cat_plot}
#| column: screen-inset
#| fig-height: !expr fig_height(new_cat_chars_train_long)

plot_small_multiple_histograms(new_cat_chars_train_long, stat = "count")
```

```{r feature_dist_train_cat_cleanup}
#| output: false

rm(new_cat_chars_train_long)
gc()
```

:::

# Distribution of SHAPs (Categorical)

```{r shap_dist_cat_transform}
#| eval: !expr isTRUE(shap_exists)

new_cat_shap_long <- new_shaps %>%
  select(meta_pin, meta_card_num, all_of(categorical_preds)) %>%
  pivot_longer(
    cols = all_of(categorical_preds),
    names_to = "predictor",
    values_to = "value"
  ) %>%
  left_join(
    new_shaps %>%
      select(meta_pin, meta_card_num, all_of(categorical_shaps)) %>%
      pivot_longer(
        cols = all_of(categorical_shaps),
        names_to = "shap_name",
        values_to = "shap_value"
      ) %>%
      mutate(shap_name = str_replace(shap_name, "_shap$", "")),
    by = c("meta_pin", "meta_card_num", "predictor" = "shap_name")
  )
```
```{r create_fig_height_shap_cat}
fig_height_shap <- if (exists("new_cat_shap_long")) {
  fig_height(new_cat_shap_long, ncol = ncol_violin)
} else {
  4
}
```

```{r shap_dist_cat_plot}
#| eval: !expr isTRUE(shap_exists)
#| column: screen-inset
#| fig-height: !expr fig_height_shap

plot_small_multiple_violins(
  new_cat_shap_long,
  "shap_value",
  y_axis_label = "SHAP Value"
)
```

```{r shap_dist_cat_cleanup}
#| eval: !expr isTRUE(shap_exists)
#| output: false

rm(new_cat_shap_long)
gc()
```
