...
comp_cat_chars_assess_long <- comp_assessment_data %>%
...
plot_small_multiple_histograms(comp_cat_chars_assess_long,
  baseline_cat_chars_assess_long,
  stat = "count"
)
```
...
rm(comp_cat_chars_assess_long, baseline_cat_chars_assess_long)
gc()
```
...
comp_cat_chars_train_long <- comp_training_data %>%
  select(all_of(categorical_preds)) %>%
  mutate(across(everything(), as.character)) %>%
  pivot_longer(
    cols = everything(),
...
plot_small_multiple_histograms(comp_cat_chars_train_long,
  baseline_cat_chars_train_long,
  stat = "count"
)
```
...
rm(comp_cat_chars_train_long, baseline_cat_chars_train_long)
gc()
```
...
rm(comp_cat_shap_long, baseline_cat_shap_long)
gc()
```
