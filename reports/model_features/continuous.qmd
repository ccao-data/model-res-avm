...
comp_cont_chars_assess_long <- comp_assessment_data %>%
  select(all_of(continuous_preds)) %>%
  pivot_longer(
    cols = everything(),
    names_to = "predictor",
...
plot_small_multiple_histograms(comp_cont_chars_assess_long,
  baseline_cont_chars_assess_long,
  stat = "bin"
)
```
...
rm(baseline_cont_chars_assess_long, comp_cont_chars_assess_long)
gc()
```
...
comp_cont_chars_train_long <- comp_training_data %>%
  select(all_of(continuous_preds)) %>%
  pivot_longer(
    cols = everything(),
    names_to = "predictor",
...
plot_small_multiple_histograms(comp_cont_chars_train_long,
  baseline_cont_chars_train_long,
  stat = "bin"
)
```
...
rm(baseline_cont_chars_train_long, comp_cont_chars_train_long)
gc()
```
...
comp_cont_shap_long <- comp_shaps %>%
  select(meta_pin, meta_card_num, all_of(continuous_preds)) %>%
  pivot_longer(
    cols = all_of(continuous_preds),
    names_to = "predictor",
...
if (exists("comp_cont_shap_long") && exists("baseline_cont_shap_long")) {
...
rm(comp_cont_shap_long, baseline_cont_shap_long)
gc()
```
