...
common_cols <- intersect(names(assessment_data_old), names(assessment_data_new))

card_data <- bind_rows(
  old = assessment_data_old %>% select(all_of(common_cols)),
  new = assessment_data_new %>% select(all_of(common_cols)),
  .id = "dataset" # "new" or "old"
) %>%
...
changes_summary <- unmatched_values_long %>%
  ...
  summarise(
    value_old = value[dataset == "old"][1],
    value_new = value[dataset == "new"][1],
    .groups = "drop"
  ) %>%
  # Count specific old -> new change occurrences
  group_by(column, value_old, value_new) %>%
  summarise(
    n = n(),
    .groups = "drop"
  ) %>%
  ...
  group_by(column) %>%
  mutate(
    N_unique_changes = n_distinct(paste0(value_old, "->", value_new))
  ) %>%
  ungroup()
...
make_card_comp_chart <- function(col_name) {
  ...
  nm_old <- paste0(col_name, "_old")
  nm_new <- paste0(col_name, "_new")

  # Count each distinct old/new pair and keep one example PIN/card per pair
  pair_counts <- chart_data_wide[
    ,
    .(n = .N, meta_pin = meta_pin[1], meta_card_num = meta_card_num[1]),
    by = c(nm_old, nm_new)
  ]

  # Reorder columns for readability
  order <- c("meta_pin", "meta_card_num", nm_old, nm_new, "n")
  present <- intersect(order, names(pair_counts))
  data.table::setcolorder(pair_counts, present)
  # Cap at 1,000 rows for readability with sample
  if (nrow(pair_counts) > 1000L) {
    pair_counts <- pair_counts[sample(.N, 1000L)]
  }
  pair_counts[]
}
