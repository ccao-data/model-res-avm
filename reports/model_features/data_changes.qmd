...
common_cols <- intersect(names(old_chars), names(new_assessment_data))

card_data <- bind_rows(
  comp = old_chars %>% select(all_of(common_cols)),
  baseline = new_assessment_data %>% select(all_of(common_cols)),
