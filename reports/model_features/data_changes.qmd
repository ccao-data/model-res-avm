...
common_cols <- intersect(
  names(comp_assessment),
  names(baseline_assessment_data)
)

card_data <- bind_rows(
  comp = comp_assessment %>% select(all_of(common_cols)),
...
Many of our features should be exact matches but are not. For example, distance 
to features have some minor variation due to geocoding variance.
Because of this, all distance features have a buffer of 5 feet.
There are also features which should not match, but we want to make sure that
they stay relatively stable. For example, the percentage of college educated
individuals should not change by more than 10% year over year.
...
make_card_comp_chart <- function(col_name) {
  ...
  # One row per distinct change (old/new),
  # with arbitrary example PIN/card + count
  distinct_changes <- chart_data_wide[
    ,
    .(
      meta_pin = meta_pin[1],
      meta_card_num = meta_card_num[1],
      n = .N
    ),
    by = c(old_nm, new_nm)
  ]
  ...
  present <- intersect(order, names(distinct_changes))
  data.table::setcolorder(distinct_changes, present)

  # Cap at 1,000 distinct changes for readability
  if (nrow(distinct_changes) > 1000L) {
    distinct_changes <- distinct_changes[sample(.N, 1000L)]
  }
