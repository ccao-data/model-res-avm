...
common_cols <- intersect(names(old_assessment_data), names(new_assessment_data))

card_data <- bind_rows(
  old = old_assessment_data %>% select(all_of(common_cols)),
  new = new_assessment_data %>% select(all_of(common_cols)),
...
changes_summary <- unmatched_values_long %>%
  ...
  summarise(
    old_value = value[dataset == "old"][1],
    new_value = value[dataset == "new"][1],
    .groups = "drop"
  ) %>%
