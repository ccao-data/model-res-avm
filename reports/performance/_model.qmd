...
est_v_actual_plotly_town(assessment_data, pred_card_initial_fmv)
```
...
model_big_misses_assessment <- assessment_pin %>%
  ...
  mutate(
    Difference = (`Sale 1 Price` - `Est. FMV`)),
    `Qnt.` = cut(
      `Sale 1 Price`,
...
```
### Assessment Set (All Years)

```{r _model_big_misses_assessment_all}
model_big_misses_assessment_all <- assessment_pin %>%
  mutate(township_name = ccao::town_convert(meta_township_code)) %>%
  filter(
    meta_triad_code == run_triad_code,
    !is.na(sale_recent_1_price),
...
model_big_misses_assessment_all %>%
  datatable(
    rownames = FALSE,
    options = list(
      columnDefs = list(
