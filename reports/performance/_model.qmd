...
```{r _model_lorenz_curve}
gini_data <- assessment_pin %>%
  filter(!is.na(sale_recent_1_price) & sale_recent_1_date > as.numeric(params$year) - 1) %>%
  select(pred_pin_final_fmv_round_no_prorate, sale_recent_1_price, meta_township_code) %>%
  arrange(sale_recent_1_price) %>%
  left_join(ccao::town_dict, by = c("meta_township_code" = "township_code"))

create_lorenz_curve <- function(data, township) {
  sale_price <- data$sale_recent_1_price
  assessed <- data$pred_pin_final_fmv_round_no_prorate

  gini_sale_price <- gini(sale_price)
  gini_assessed <- gini(assessed)
  ...
  lorenz_data <- rbind(lorenz_data_price, lorenz_data_assessed)

  ggplot(lorenz_data, aes(x = cum_pct, y = pct, color = type)) +
    geom_line() +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "green") +
    scale_color_manual(values = c("Sale GINI" = "red", "Assessed GINI" = "blue")) +
    scale_x_continuous(labels = scales::label_percent()) +
    scale_y_continuous(labels = scales::label_percent()) +
    labs(
      title = paste("Lorenz Curve for", township),
...
plots <- gini_data %>%
  ...
  group_nest() %>%
  mutate(plot = map2(data, township_name, ~ create_lorenz_curve(.x, .y))) %>%
  pull(plot)
