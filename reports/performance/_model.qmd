...
training_data_monthly <- training_data_pred %>%
  ...
  summarize(
    ...
    sse = sum(squared_difference),  # Sum of Square Error
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(
    variance_sale = variance_sale,
    variance_fmv = variance_fmv,
    total_sales_overall = sum(n),
    total_sse_overall = sum(sse),
    percent_sales = total_sales_overall / n * 100,
    percent_sse = sse / total_sse_overall * 100,
...
```
### Total $ Variance of Sale Price and FMV
```{r}
ggplot(training_data_monthly_long %>% filter(Metric %in% c("variance_sale", "variance_fmv")),
  ...
  scale_color_discrete(
    labels = c("variance_sale" = "Variance of Sale Price", 
  ...
  theme_minimal()
```
### Difference Between Variance of FMV and Sale Price

```{r}
ggplot(training_data_monthly, aes(x = date, y = abs(variance_diff))) +
  geom_line() +
  geom_point() +
