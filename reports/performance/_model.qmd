...
```{r _model_setup}
# Predict with the bare model fit, then apply the saved post-processing tailor
# (if present) to convert predictions back to raw dollars. Runs without a
# tailor artifact were never log-transformed, so no conversion is needed
training_data_pred <- training_data %>%
  mutate(
    pred_card_initial_fmv = {
      ...
      if (!is.null(model_post)) pred <- predict(model_post, pred)
      pred$.pred
    }
  )
