...
```{r _model_metadata}
# Chunk to populate the metadata / dataset summary under the top header
...
m_train_n_outliers_prop <-
  ((nrow(training_data) / sum(training_data$sv_is_outlier)) / 100) %>%
  scales::percent(accuracy = 0.01)
...
_NOTE: Click any of the dots on the plot below to open the Assessor's website for that PIN_
...
The following table shows shows the largest misses from the **assessment set**. It finds four sales (one per quartile of price) for each township where the model's estimate was the furthest from the actual sale price.
