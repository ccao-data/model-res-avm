...
gini_data <- assessment_pin %>%
  filter(!is.na(sale_recent_1_price) &
           sale_recent_1_date > as.numeric(params$year) - 1) %>%
  select(pred_pin_final_fmv_round_no_prorate,
         sale_recent_1_price,
         meta_township_code) %>%
...
create_lorenz_curve <- function(data, township_name) {
  ...
  calculate_gini <- function(lorenz_data) {
    sum_area <-
      sum(with(lorenz_data, base::diff(cum_pct) * (pct[-length(pct)] + pct[-1]) / 2))
  ...
  ggplot() +
    geom_line(data = lorenz_data_assessed, aes(x = cum_pct, y = pct, color = "Assessed Values")) +
    geom_line(data = lorenz_data_price, aes(x = cum_pct, y = pct, color = "Sale Values")) +
    geom_abline(
      intercept = 0,
      slope = 1,
      linetype = "dashed",
    ...
    labs(
      ...
      caption = paste("GINI Coefficient for Assessed Value is Sorted by Sale Price\nAssessed GINI:", round(gini_assessed, 3), "- Sale GINI:", round(gini_price, 3)),
