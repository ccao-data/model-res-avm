...
create_lorenz_curve <- function(data, township_name) {
  ...
  ggplot() +
    geom_line(data = lorenz_data_price, aes(x = cum_pct, y = pct), color = "blue") +
    geom_line(data = lorenz_data_assessed, aes(x = cum_pct, y = pct), color = "red") +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "green") +
    scale_x_continuous(labels = scales::label_percent()) +
    scale_y_continuous(labels = scales::label_percent()) +
    labs(title = paste("Lorenz Curve for", township_name),
...
```{r _model_lorenz_curve_results, results = 'asis'}


# Iterate over the plots and print headers with township names
plots %>%
  imap(~ {
    cat("## ", .y, "\n\n")
    print(.x)
