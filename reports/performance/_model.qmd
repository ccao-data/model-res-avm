...
Overall model performance statistics for three sets:

- **Test Set**: Predictions made on a holdout set of data that the model didn't see during training. Typically the last 10% of sales.
- **Linear Baseline**: Test set results using a separate simple linear model.
- **Assessment Set**: Predictions made on prior year sales that the model has already seen.
...
```{r _model_ratio_distribution_overall}
...
```{r _model_ratio_distribution_by_township}
