...
quantiles <- outlier_dist %>%
  group_by(meta_township_code) %>%
  summarize(q01 = quantile(ratio, 0.01, na.rm = TRUE), q99 = quantile(ratio, 0.99, na.rm = TRUE))

# Joining the quantiles back to outlier_dist
outlier_dist <- outlier_dist %>%
  left_join(quantiles, by = "meta_township_code") %>%
  mutate(
