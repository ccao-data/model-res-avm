...
training_data <- training_data %>%
  mutate(
    outlier_reasons_to_graph = case_when(
      is_outlier & !is.na(sv_outlier_reason2) ~
        paste0(sv_outlier_reason1, " (", sv_outlier_reason2, ")"),
      is_outlier & is.na(sv_outlier_reason2) ~ sv_outlier_reason1,
      TRUE ~ NA_character_
    )
  )
...
training_data %>%
  summarise(
    percent = mean(case_when(is_outlier ~ 1, TRUE ~ 0)),
    .by = meta_class
  ) %>%
...
y_lim_axis_outlier_townships <- training_data %>%
  ...
  summarise(percent = mean(case_when(is_outlier == TRUE ~ 1, TRUE ~ 0))) %>%
  ungroup() %>%
...
training_data %>%
  ...
  mutate(Category = ifelse(is_outlier, "Outlier", "Not Outlier")) %>%
...
training_data %>%
  mutate(
    Category = ifelse(is_outlier, "Outlier", "Not Outlier"),
...
outliers_ratio_comparison <- training_data %>%
  summarise(
    ...
    .by = c(is_outlier, meta_township_name)
  ) %>%
  arrange(meta_township_name, is_outlier) %>%
  group_by(meta_township_name) %>%
  mutate(
...
training_data %>%
  ...
  summarise(
    percent = mean(case_when(is_outlier == TRUE ~ 1, TRUE ~ 0)),
...
outlier_decile_breakout <- function(data, dec) {
  outlier_decile_y_axis_lim <- data %>%
    arrange(meta_sale_price) %>%
    mutate(decile = ntile(meta_sale_price, 10)) %>%
    filter(decile == dec & is_outlier == TRUE) %>%
