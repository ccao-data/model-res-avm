...
This table excludes any PINs which had a prior year assessed value of less than $35,000. It then identifies the 5 PINs in each neighborhood which had the largest year over year increase in assessed value.

```{r _outliers_table_of_largest_change}
datatable(
  largest_increases %>%
    distinct(meta_pin, .keep_all = TRUE) %>%
    filter(prior_near_tot > 35000) %>%
