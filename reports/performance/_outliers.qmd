...
qnt_df <- assessment_pin %>%
  ...
  mutate(
    Difference = abs(`Sale 2 Price` - `Est. FMV`),
    `Qnt.` = cut(
      `Sale 2 Price`,
...
This section looks at the year over year change in assessed housing values. It separates the data into individual observations, identifying PINs in each neighborhood/township which grew at the fastest rate as well as the median rate of change for each neighborhood. 
...
largest_increases <- assessment_pin %>%
  ...
  mutate(neighborhood_median_change = median(prior_near_yoy_change_pct * 100), ) %>%
...
color_palette <-
  colorNumeric(palette = c("white", "red"), domain = c(min(largest_increases$prior_near_yoy_change_pct), 300))
...
leaflet() %>%
  ...
  addCircleMarkers(
    ...
    popup = ~ paste(
      "Neighborhood:",
      meta_nbhd_code,
      "<br>Current year fair market value rounded:",
      pred_pin_final_fmv_round,
      "<br>Previous reassessed value:",
      prior_near_tot,
      "<br>% Change:",
...
labels <- c("< 0%", "0% - 50%", "50% - 100%", "100% - 150%", "150% - 200%", "200% - 250%", "250% - 300%", "> 300%")
...
my_colors <- c("red", "green", "blue", "orange", "yellow", "pink", "purple", "grey")
...
leaflet(data = nbhd_increases) %>%
  ...
  addPolygons(
    ...
    fillOpacity = 0.7,
    smoothFactor = 0.5,
    label = ~ paste0("Change: ", round(neighborhood_median_change, 2), "% (Category ", change_category, ")"),
  ...
  addLegend(
    ...
    title = "Neighborhood Level Change",
    opacity = 1
  )
```
