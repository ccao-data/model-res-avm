...
```{r}
assessment_card %>%
  summarize('NA Initial FMV' = sum(is.na(pred_card_initial_fmv)),
...
```{r}
assessment_pin %>%
  summarize('NA Initial FMV' = sum(is.na(pred_pin_initial_fmv)),
            ...
            'Negative Final Building FMV' = sum(pred_pin_final_fmv_bldg < 0, na.rm = TRUE))
```
:::
