  ...
  mutate(
    sv_outlier_reason_simplified = case_when(
      str_detect(sv_outlier_reason, regex("High price", ignore_case = TRUE)) &
        str_detect(sv_outlier_reason, regex("PTAX-203 Exclusion", ignore_case = TRUE)) ~ # nolint
...
training_data_outlier_analysis %>%
  filter(sv_is_outlier) %>%
  group_by(meta_year, sv_outlier_reason_simplified) %>%
  summarise(n = n()) %>%
...
training_data_outlier_analysis %>%
  # Remove preoperties that don't come out of the box with sv_is_outlier
  # classifications, such as multi-card properties
  filter(!is.na(sv_is_outlier)) %>%
  summarise(
...
```{r}
# TODO:
# - make sure the same exclusion filters (multi-card, etc) are set before doing this analysis
...
# 3) Summaries by (triad, nbhd)
#    - overall outlier proportion: mean(is_outlier)
#    - total sales: n()
#    - reason breakdown: HTML list per nbhd
  ...
  summarise(
    reason_html = paste0(
      ...
      paste0(
        ...
        ": <strong>", n_reason, "</strong> (", sprintf("%.1f%%", pct_of_total * 100), ")",
        "</li>",
        collapse = ""
      ),
      "</ul>"
...
labels <- vapply(
  ...
  function(i) {
    low <- breaks[i] * 100
    high <- breaks[i + 1] * 100
    if (is.infinite(high)) paste0("≥ ", format_pct(low)) else paste0(format_pct(low), " – ", format_pct(high))
  },
  character(1)
)
  ...
  map_data <- nbhd_sf %>%
    ...
    mutate(
      popup_html = ifelse(
        is.na(outlier_prop),
        paste0(
          "<strong>", htmlEscape(town_nbhd), " - ", htmlEscape(township_name), "</strong><br>",
...
triads <- sort(unique(na.omit(training_data_outlier_analysis$meta_triad_name)))
triads
