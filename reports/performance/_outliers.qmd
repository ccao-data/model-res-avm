...
This map uses the test_card dataset to calcualte the ratio of sale price to the initial fair market value. It filters the the data to where our estimate is abnormally high (>99th percentile) and abnormally low (<1st percentile).

```{r outlier_map, message=FALSE}
# Calculating the ratio
outlier_dist <- test_card %>%
  mutate(
    ratio = meta_sale_price / pred_card_initial_fmv,
    q01 = quantile(ratio, 0.01, na.rm = TRUE),
    q99 = quantile(ratio, 0.99, na.rm = TRUE),
    .by = meta_township_code
  ) %>%
  # Calculating quantiles for each group
  mutate(
    quantile_category = case_when(
      ratio <= q01 ~ "Abnormally Low Outlier",
      ratio >= q99 ~ "Abnormally High Outlier",
      TRUE ~ "1st-99th Percentile"
    )
  ) %>%
  left_join(
    training_data %>%
      select(meta_pin, loc_latitude, loc_longitude),
    by = "meta_pin"
  ) %>%
  filter(
    meta_triad_code == as.numeric(run_triad_code),
    quantile_category != "1st-99th Percentile",
    !is.na(loc_longitude) & !is.na(loc_latitude)
  ) %>%
