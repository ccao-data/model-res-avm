...
## Mean Comparison Score for Properties in Each Neighborhood
```{r}
...
plots <- list()
plots$"Overall" <- generate_plot(mean_comp, "Overall Mean Comparison")
...
comp_df_filtered <- comp_df %>%
  ...
  group_by(comp_pin) %>%
  left_join(select(assessment_pin, meta_pin, pred_pin_final_fmv_round_no_prorate), by = c("comp_pin" = "meta_pin")) %>%
...
Target_pin <- comp_df %>%
  ...
  left_join(select(assessment_pin, meta_pin, pred_pin_final_fmv_round_no_prorate, sale_recent_1_date, sale_recent_1_price), by = c("pin" = "meta_pin")) %>%
  mutate(comp_pin = pin) %>%
...
leaflet() %>%
  ...
  addCircleMarkers(
    ...
    popup = ~ paste0(
      ...
      "<br>Model FMV: ", scales::dollar(pred_pin_final_fmv_round_no_prorate, accuracy = 1),
      "<hr>",
