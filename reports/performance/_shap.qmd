...
The following map demonstrates the impact of 20 key indicators on neighborhood-level housing values. Colors in the map are scaled to each characteristics percentage of *total absolute values* for all characteristics. 
```{r _shap_township_map}
neighborhoods <- ccao::nbhd_shp

chars_data_1year <- chars_data %>%
  filter(meta_year == max(meta_year)) %>%
  select(meta_pin, meta_nbhd_code) %>%
  rename(nbhd_code = meta_nbhd_code) %>%
  distinct(meta_pin, .keep_all = TRUE)


absolute_values <- shap_df_filtered %>%
  mutate(across(where(is.numeric), abs)) %>%
  left_join(chars_data_1year, by = "meta_pin") %>%
  mutate(total_abs = rowSums(select(., all_of(shap_predictors)), na.rm = TRUE)) %>%
  group_by(nbhd_code) %>%
  summarise(mean(total_abs))
...
Shap_map_dataset <- shap_df_filtered %>%
  ...
  group_by(nbhd_code) %>%
  mutate(across(where(is.numeric), abs)) %>%
  summarise_at(vars(all_of(shap_predictors)), ~ mean(.), na.rm = TRUE) %>%
  mutate(nbhd_code = as.character(nbhd_code)) %>%
  left_join(neighborhoods, by = c("nbhd_code" = "town_nbhd")) %>%
  left_join(absolute_values, by = c("nbhd_code" = "nbhd_code")) %>%
  st_as_sf()
...
Shap_map_dataset <- Shap_map_dataset %>%
  select(
    ...
    "Total of Absolute Values" = "mean(total_abs)"
  )

shap_map <- leaflet(Shap_map_dataset) %>%
  addProviderTiles(providers$CartoDB.Positron)

add_column_layer <- function(df, column_name, map, total_value) {
  # Define the color palette function
  pal <- colorNumeric(palette = c("white", "red"), domain = df[[column_name]] / total_value * 100, na.color = "transparent")

  # Add polygon layer using this color palette
  map %>% addPolygons(
    fillColor = ~ pal(df[[column_name]] / total_value * 100),
    weight = 1,
    opacity = 1,
    color = "black",
    fillOpacity = 0.7,
    group = column_name,
    label = ~ paste(
      column_name, ": ", label_dollar(accuracy = 1)(df[[column_name]]),
      sprintf("(%.2f%%)", df[[column_name]] / total_value * 100)
    ),
...
total_value <- Shap_map_dataset$`Total of Absolute Values`
