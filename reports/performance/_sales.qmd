...
sales_volume_town_year_plot <- training_data %>%
  ...
  summarise(
    num_sales = sum(!is_outlier, na.rm = TRUE),
...
training_data %>%
  mutate(
    outlier = ifelse(is_outlier, "Outlier", "Not Outlier"),
    outlier = factor(outlier, c("Outlier", "Not Outlier"))
  ) %>%
  ggplot(aes(x = meta_year, group = is_outlier)) +
...
training_data %>%
  ...
  ggplot(aes(x = time_sale_month_of_year, group = is_outlier)) +
  geom_bar(aes(fill = outlier)) +
