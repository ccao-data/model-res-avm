...
stats_county_logit <- stats_logit_fit %>%
  ...
  pack_rows("Exterior Wall Material", 49, 52) %>%
  pack_rows("Central Heating", 53, 56) %>%
  pack_rows("Basement Finish", 57, 59) %>%
  pack_rows("Roof Material", 60, 65) %>%
  pack_rows("Class", 66, 81) %>%
  pack_rows("Porch", 82, 83) %>%
  pack_rows("Central Air Conditioning", 84, 85) %>%
  pack_rows("Design Plan", 86, 87) %>%
  pack_rows("Recent Renovation", 88, 88) %>%
  pack_rows("ACS5", 89, 94)
```
