...
```{r model_perf, message=FALSE, fig.align="center"}
...
# reporting/current year. See wiki SOPs
sales_data_wide <- read_parquet(here("input", "modeldata.parquet")) %>%
  filter(ind_arms_length, meta_year >= report_year - 2) %>%
  group_by(meta_pin) %>%
  filter(meta_sale_date == max(meta_sale_date)) %>%
  select(meta_pin, meta_year, meta_class, meta_multi_code, meta_sale_price) %>%
  ungroup() %>%
  mutate(meta_year = case_when(
    meta_year == report_year ~ "yr-0",
    meta_year == report_year - 1 ~ "yr-1",
    meta_year == report_year - 2 ~ "yr-2"
  )) %>%
  pivot_wider(
...
# Calculate COD, PRD, PRB, and median ratio for each set of available values
pv_model_perf <- pv_final_values %>%
  ...
  group_by(town_name) %>%
  summarize(
    bor_cod = ccao_cod(pri_yr_board_est / `sale_price_yr-2`, na.rm = TRUE)$COD,
    bor_prd = ccao_prd(pri_yr_board_est, `sale_price_yr-2`, na.rm = TRUE)$PRD,
    bor_prb = ccao_prb(pri_yr_board_est, `sale_price_yr-2`, na.rm = TRUE)$PRB,
    bor_med = median(pri_yr_board_est / `sale_price_yr-2`, na.rm = TRUE),
...
pv_model_perf_headers <- htmltools::withTags(table(
  ...
  thead(
    tr(
      ...
      th(colspan = 4, glue("BoR ({report_year - 1})")),
      th(colspan = 4, glue("Certified ({report_year})")),
      th(colspan = 4, "Initial Model"),
      th(colspan = 4, "Final Model"),
    ),
    tr(
...
# Create an interactive datatable with all stats
pv_model_perf %>%
  datatable(
    rownames = FALSE,
    filter = "none",
  ...
  formatStyle(  
    pv_model_perf %>% select(2:17) %>% names() %>% sort(),
    pv_model_perf %>% select(18:33) %>% names() %>% sort(),
