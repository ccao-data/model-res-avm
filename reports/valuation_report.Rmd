...
# Save assessment and sales year, first loading default params from env,
# then from report params, then from data
assessment_year <- Sys.getenv("R_ASSESSMENT_YEAR", unset = NA)
assessment_year <- case_when(
  !is.na(as.numeric(assessment_year)) ~ as.numeric(assessment_year),
  !is.na(as.numeric(params$assessment_year)) ~ as.numeric(params$assessment_year),
  TRUE ~ lubridate::year(max(pv_final_values$meta_sale_date))
)
sales_year <- assessment_year - 1
...
sales_data <- read_parquet(here("input", "modeldata.parquet")) %>%
  filter(ind_arms_length, meta_year >= sales_year - 2) %>%
  group_by(meta_pin) %>%
  filter(meta_sale_date == max(meta_sale_date)) %>%
  distinct(meta_pin, meta_sale_price, meta_document_num, .keep_all = TRUE) %>%
  ungroup() %>%
  select(
    meta_pin, meta_year, meta_town_code, meta_class, meta_multi_code,
    meta_document_num, meta_sale_price
  ) %>%
...
sales_data_wide <- sales_data %>%
  ...
  mutate(meta_year = case_when(
    meta_year == sales_year ~ "yr-0",
    meta_year == sales_year - 1 ~ "yr-1",
    meta_year == sales_year - 2 ~ "yr-2"
  )) %>%
  pivot_wider(
...
pv_calc_stats <- function(data) {
  data %>%
  summarize(
    bor_cod = ccao_cod(pri_yr_board_est / `sale_price_yr-0`, na.rm = TRUE)$COD,
    bor_prd = ccao_prd(pri_yr_board_est, `sale_price_yr-0`, na.rm = TRUE)$PRD,
    bor_prb = ccao_prb(pri_yr_board_est, `sale_price_yr-0`, na.rm = TRUE)$PRB,
    bor_med = median(pri_yr_board_est / `sale_price_yr-0`, na.rm = TRUE),
    
    cert_cod = ccao_cod(certified_est / `sale_price_yr-1`, na.rm = TRUE)$COD,
    cert_prd = ccao_prd(certified_est, `sale_price_yr-1`, na.rm = TRUE)$PRD,
    cert_prb = ccao_prb(certified_est, `sale_price_yr-1`, na.rm = TRUE)$PRB,
    cert_med = median(certified_est / `sale_price_yr-1`, na.rm = TRUE),
    
    init_cod = ccao_cod(lgbm_value / `sale_price_yr-0`, na.rm = TRUE)$COD,
    init_prd = ccao_prd(lgbm_value, `sale_price_yr-0`, na.rm = TRUE)$PRD,
    init_prb = ccao_prb(lgbm_value, `sale_price_yr-0`, na.rm = TRUE)$PRB,
    init_med = median(lgbm_value / `sale_price_yr-0`, na.rm = TRUE),
    
    final_cod = ccao_cod(final_value / `sale_price_yr-0`, na.rm = TRUE)$COD,
    final_prd = ccao_prd(final_value, `sale_price_yr-0`, na.rm = TRUE)$PRD,
    final_prb = ccao_prb(final_value, `sale_price_yr-0`, na.rm = TRUE)$PRB,
    final_med = median(final_value / `sale_price_yr-0`, na.rm = TRUE),
  ) %>%
  mutate(
...
pv_model_perf_triad <- pv_final_values %>%
  left_join(sales_data_wide, by = c("meta_pin", "meta_class", "meta_multi_code")) %>%
  filter(!ind_multi_pin, pri_yr_board_est != 0, certified_est != 0) %>%
  group_by(triad) %>%
  pv_calc_stats()
...
pv_model_perf_headers <- htmltools::withTags(table(
  ...
  thead(
    tr(
      ...
      th(colspan = 4, glue("BoR ({sales_year - 1}) / {sales_year} Sales")),
      th(colspan = 4, glue("Certified ({sales_year}) / {sales_year - 1} Sales")),
      th(colspan = 4, glue("Initial Model / {sales_year} Sales")),
      th(colspan = 4, glue("Final Model / {sales_year} Sales")),
    ),
    tr(
...
pv_summary_headers <- htmltools::withTags(table(
  ...
  thead(
    tr(
      ...
      th(rowspan = 2, glue("Num. Sales ({sales_year})")),
      th(colspan = 8, "Median Values"),
    ),
    tr(
      th(glue("BoR Value ({sales_year - 1})")),
      th(glue("Certified Value ({sales_year})")),
      th(glue("Sale Price ({sales_year})")),
...
ccao::town_shp %>%
  inner_join(pv_summary, by = c("township_name" = "town_name")) %>%
  pivot_longer(
...
tibble(
  ...
  "Min. Turnover" = c(pv_model$ntile_min_turnover, ""),
...
# Load the set of sales that were used to generate post-modeling values
# This is usually all sales from 2 years before the assessment date
sales_data_prev_2_years <- sales_data %>%
  filter(meta_year >= assessment_year - 2) %>%
  distinct(meta_pin, meta_sale_price)



# Join the sales sample, then assign a post-modeling ntile to each estimated
# value. Each ntile has an adjustment percentage stored in the postval_model obj
pv_postmodel_vals <- pv_final_values %>%
  ...
  mutate(ntile = val_assign_ntile(lgbm_value, ntiles_lst))
...
pv_postmodel_vals2 <- pv_postmodel_vals %>%
  ...
  # exists
  mutate(
    value_ntile_adj = rowSums(tibble(lgbm_value, lgbm_value * ntile_med_pct_adj), na.rm = T),
    value_cap_adj = val_limit_ratios(meta_sale_price, value_ntile_adj),
  ) %>%
  
  # Join townhome adjustments and overwrite ntile adjustments for townhomes only
  left_join(pv_model$townhome_adjustments) %>%
  mutate(
    value_th_adj = ifelse(!is.na(th_final_value), th_final_value, value_cap_adj),
...
qnums <- pv_model$ntile_adjustments %>%
  ...
  group_by(meta_town_code, meta_nbhd, meta_modeling_group) %>%
...
pv_postmodel_summary2 <- pv_postmodel_vals2 %>%
  ...
  summarize(
    pct = median(na_if((value_ntile_adj - lgbm_value) / lgbm_value, 0), na.rm = T),
...
pv_postmodel_headers2 <- htmltools::withTags(table(
  ...
  thead(
    tr(
      th(rowspan = 2, "Township"),
      th(colspan = 5, "Count Adj. (All Props.)"),
      th(colspan = 5, "Median Pct. Adj."),
...
## Ratio Density Distribution by Township
...
pv_dist_plot <- function(data, lims) {
  data %>%
    ggplot() +
...
towns <- pv_final_values %>% 
  group_by(town_name) %>%
  summarize(med_val = median(final_value, na.rm = T)) %>%
  arrange(med_val) %>%
  pull(town_name)
...
lapply(towns, function(chunk) pv_dist_plot(
  ...
  pv_final_values %>% filter(town_name %in% chunk) %>% pull(final_value) %>% quantile(., probs = c(0.05, 0.95))
))
```
...
decile_data_yoy <- pv_final_values %>%
  filter(
    !is.na(board_yoy_pct_change), !is.na(pri_yr_board_est), pri_yr_board_est != 0,
  ...
  mutate(
    name = case_when(
      name == "board_yoy_pct_change" ~ glue("BoR ({sales_year - 1}) to Final Model"),
      name == "certified_yoy_pct_change" ~ glue("Cert. ({sales_year}) to Final Model")
    ),
    name = factor(name, levels = c(
      glue("Cert. ({sales_year}) to Final Model"),
      glue("BoR ({sales_year - 1}) to Final Model")
