...
pv_final_values <- read_parquet(
  ...
  filter(meta_modeling_group != "BB", !ind_multi_pin) %>%
  mutate(
...
pv_model_perf_triad <- pv_final_values %>%
  ...
  filter(!ind_multi_pin, pri_board_est != 0, certified_est != 0) %>%
  group_by(triad) %>%
  pv_calc_stats()
...
pv_model_perf_headers <- htmltools::withTags(table(
  ...
  thead(
    tr(
      ...
      th(colspan = 4, glue("BoR ({assessment_year - 3}) / {assessment_year - 4} Sales")),
      th(colspan = 4, glue("Certified ({assessment_year - 1}) / {assessment_year - 2} Sales")),
      th(colspan = 4, glue("Initial Model / {assessment_year - 1} Sales")),
      th(colspan = 4, glue("Final Model / {assessment_year - 1} Sales")),
    ),
    tr(
...
qnums <- pv_model$ntile_adjustments %>%
  ...
  group_by(meta_town_code, meta_nbhd, meta_class) %>%
...
decile_data_yoy <- pv_final_values %>%
  ...
  mutate(
    name = case_when(
      name == "board_yoy_pct_change" ~ glue("BoR ({assessment_year - 4}) to Final Model"),
      name == "certified_yoy_pct_change" ~ glue("Cert. ({assessment_year - 1}) to Final Model")
    ),
