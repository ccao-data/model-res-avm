...
pin_sale <- dbGetQuery(
  conn = con,
  sprintf("SELECT year, pin, num_parcels_sale, sv_outlier_type, doc_no, sale_date, sale_price FROM default.vw_pin_sale WHERE CAST(year AS INTEGER) >= %d AND pin IN %s", as.numeric(params$year) - 10, pins_clause)
)
