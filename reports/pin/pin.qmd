...
char_table_data %>% 
  ...
  mutate(
    ...
    "Elementary School District" = char_table_data$loc_school_elementary_district_geoid,
    "Secondary School District" = char_table_data$loc_school_secondary_district_geoid,
    
    "Bedrooms" = as.character(char_table_data$char_beds),
    "Bathrooms" = glue("{char_table_data$char_fbath} Full / {char_table_data$char_hbath} Half", .na = NULL),
    
    "Basement" = glue("{char_table_data$char_bsmt}, {char_table_data$char_bsmt_fin}", .na = NULL),
    "Garage Size" = char_table_data$char_gar1_size,
    
    "Porch" = char_table_data$char_porch, 
    "Facade" = char_table_data$char_ext_wall,
    
    "HVAC" = glue("{char_table_data$char_heat}, {char_table_data$char_air}", .na = NULL),
  ...
  tab_footnote(
    footnote = "Full bath defined as bathrooms with a shower or bathtub. Half bath
    defined as bathroom without a shower or bathtub.",
...
assessment_pin %>%
  ...
  cols_hide(columns = c(sale_recent_1_date, sale_recent_2_date,
...
multi_data <- assessment_card %>% 
  ...
  mutate(
    ...
    `Proration Rate` = scales::percent(meta_tieback_proration_rate, accuracy = 0.01),
    `Card %` = scales::percent(meta_card_pct_total_fmv, accuracy = 0.01), 
    #`Card Final` = scales::dollar(pred_card_final_fmv)
  ...
  multi_data %>% 
    ...
    tab_footnote(
      footnote = "Percentage of total PIN building value an individual card is worth",
  ...
  cols_hide(columns = c(meta_pin, char_class, meta_card_num, meta_card_pct_total_fmv,
                        char_bldg_sf, pred_card_initial_fmv, pred_card_final_fmv,
                        meta_complex_id, meta_1yr_pri_board_tot, 
                        meta_tieback_proration_rate, prior_far_tot, prior_near_tot,
