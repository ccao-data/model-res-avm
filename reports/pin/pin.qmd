...
assessment_pin %>%
  ...
  select(
    "Township" = township,
    "FMV 3 Years Prior" = prior_far_tot,
    "FMV 1 Year Prior" = prior_near_tot,
  ...
  mutate(across(`FMV 3 Years Prior`:`Land Rate Effective`, scales::dollar)) %>%
  kable() %>%
  kable_styling(
    "striped",
    position = "left",
...
assessment_pin %>%
  filter(meta_pin == params$pin) %>%
  left_join(training_data[, c("meta_pin", "sv_outlier_type", "meta_sale_document_num", "meta_sale_date", "meta_sale_price")], by = "meta_pin") %>%
