...
test_res <- read_parquet(here("output", "data", "testdata_chicago_retuned.parquet")) %>%
  mutate(
...
test_res <- test_res %>% filter(!town_name %in% c("Leyden", "Maine"))
...
test_summary <- test_res %>%
  ...
  summarize(
    ...
    final = ccao_cod(stack / meta_sale_price, suppress=T)$COD,
