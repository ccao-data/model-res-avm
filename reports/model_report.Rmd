...
```{r timing}
# Print total training time if timings file exists
...
test_summary <- test_res %>%
  ...
  summarize(
    ...
    final_med_ratio = median(
      (lgbm / meta_sale_price)[
        !assessr::is_outlier(lgbm / meta_sale_price, method = "quantile")
      ]
    )
  ) %>%
  mutate(
...
outlier_dist %>%
  ...
  formatCurrency(
    c("Sale Price", "Model Prediction", "Median Income",
...
# If model file exists, calculate feature importance
...
# Load hyperparameters data from file
