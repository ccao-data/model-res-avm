...
```{r _model_sales_vs_estimates_township, fig.width=8, fig.height=14, fig.align='center'}
...
reshaped_data <- sales_vs_estimates %>%
  ...
  mutate(space_maker = as.numeric(factor(category)),
         space_maker = space_maker + (space_maker - 1) %/% 2)

line_ends <- reshaped_data %>%
  group_by(space_maker, category) %>%
  summarize(Start = min(value), End = max(value))%>%
  mutate(color_class = ifelse(grepl("Sale", category), "Sale", "Estimate"),
         # Set for legend re-ordering
         color_class = factor(color_class, levels = c("Sale", "Estimate"))) 


ggplot() +
  geom_segment(data = line_ends, 
               aes(x = Start, 
                   xend = End, 
                   y = space_maker, 
                   yend = space_maker,
                   color = color_class), 
               size = 1.2) +
  geom_point(data = reshaped_data, 
             aes(x = value, y = space_maker), 
             color = "black", 
             size = 2, 
             shape = 21, 
             fill = "white") +
  theme_minimal() +
  theme(axis.title.x = element_text(color = "darkblue", size = 12),
        axis.title.y = element_text(color = "darkblue", size = 12),
        legend.position = c(.9, .9)) +
  labs(x = "Value", 
       y = "Township", 
       title = "Sales vs Estimates by Township",
       subtitle = "25th percentile, median, 75th percentile",
       color = "") +
  scale_color_manual(values = c( "Sale" = "darkolivegreen",
                                 "Estimate" = "darkgoldenrod3")) +
  scale_y_continuous(breaks = line_ends$space_maker, 
                     labels = line_ends$category) +
  scale_x_continuous(labels = label_dollar(),
                     # Prevents last x axis tick mark from being cut off
                     limits = c(NA, max(reshaped_data$value) * 1.04))
...
```{r _sales_vs_estimates_graph, fig.width=8, fig.height=9, fig.align='center'}

sale_vs_estimate_by_class <- function(township_to_graph, data) {
  sales_vs_estimates_by_class <- data %>% 
    dplyr::filter(triad_code == run_triad_code,
           geography_type == 'township_code',
           by_class == TRUE) %>% 
    select(geography_type,
           geography_id,
  ...
  reshaped_data <- sales_vs_estimates_by_class %>%
    mutate(township = ccao::town_convert(geography_id)) %>% 
...
```{r}
sale_vs_estimate_by_class("Berwyn", model_performance_test)
```
