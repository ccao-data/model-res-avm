...
test1 <- assessment_pin %>%
  mutate(
    ...
    is_recent_sale = if_else(
      ...
      sale_recent_1_date <= as.Date(paste0(numeric_year, "-12-31")),
      1,
      0
    )
  ) %>%
  ...
  summarize(
    ...
    yoy_change_dollar = median(prior_near_yoy_change_nom, na.rm = TRUE),
    recent_sale_yoy_change_dollars = median(if_else(is_recent_sale == 1, prior_near_yoy_change_nom, NA_real_), na.rm = TRUE)
  ) %>%
  filter(complete.cases(.)) %>%
  mutate(change_ratio_percent = recent_sale_yoy_change_percent / yoy_change_percent,
         change_ratio_dollars = recent_sale_yoy_change_dollars / yoy_change_dollar)   %>%
  left_join(ccao::town_dict, by = c("meta_township_code" = "township_code")) %>%
  mutate(
    change_ratio_percent = cell_spec(change_ratio_percent, color = ifelse(change_ratio_percent > 1, "green", "red")),
    change_ratio_dollars = cell_spec(change_ratio_dollars, color = ifelse(change_ratio_dollars > 1, "green", "red"))
  ) %>%
  select(township_name, change_ratio_percent, recent_sale_yoy_change_percent, yoy_change_percent, change_ratio_dollars, recent_sale_yoy_change_dollars, yoy_change_dollar) %>%
  kable("html", escape = F) %>%
  kable_styling()
