...
assessment_pin %>%
  mutate(
    ...
    is_recent_sale = if_else(
      !is.na(sale_recent_1_date) &
        sale_recent_1_date >= as.Date(paste0(numeric_year - 3, "-01-01")),
      1,
      0
    )
  ) %>%
  ...
  mutate(
    change_ratio_percent = cell_spec(
      change_ratio_percent,
      color = ifelse(change_ratio_percent < 1.05 & change_ratio_percent > 0.95, "green", "red")
    ),
    change_ratio_dollars = cell_spec(
      change_ratio_dollars,
      color = ifelse(change_ratio_dollars < 1.05 & change_ratio_dollars > 0.95, "green", "red")
    )
  ) %>%
