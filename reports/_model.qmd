...
```{r time_trends_seen, fig.height=8, fig.width=7, out.width="100%"}
training_data_pred %>%
  filter(
    !sv_is_outlier,
...
```{r time_trends_unseen, fig.height=8, fig.width=7, out.width="100%"}
test_card %>%
  filter(meta_triad_code == run_triad_code) %>%
  mutate(
    time_interval = lubridate::interval(
