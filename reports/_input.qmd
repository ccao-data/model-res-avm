...
chars_data_fil %>%
  inner_join(hie_data_fil %>% select(-year), by = join_by(meta_pin == pin)) %>%
  left_join(
...
```{r land_valuation, out.width="100%"}
assessment_pin %>%
