...
3. `model_features.qmd` - Report that looks at model inputs. It has three sections: 1) feature changes between the previous and current model, 2) distributions of SHAP and feature values, and 3) feature missingness. This report runs based on a toggle in our aws workflow since it is computationally intensive and only necessary if input features change.
