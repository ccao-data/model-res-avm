...
township <- training_data %>%
  ...
  scale_x_continuous(breaks = seq((as.numeric(params$year) - 9), as.numeric(params$year), 1)) +
  theme_minimal() +
...
training_data %>%
  filter(!sv_is_outlier, meta_triad_name == run_triad, meta_year >= as.numeric(params$year) - 4) %>%
  mutate(
    Bin = cut(
      meta_sale_price,
...
Where are 211s and 212s increasing a *lot*? Near values are Certified values from the current year, far values are Board values from two years ago.
...
time_frame <- c("min" = as.character(as.numeric(params$year) - 1), "max" = params$year)

# Data path
data_path <- "~/model-res-avm-move-existing-reports/data/res_parcels.parquet"
...
year_minus_3 <- as.numeric(params$year) - 4

temp <- ggplot(
  sales %>%
    filter(sv_is_outlier == FALSE & meta_year >= as.numeric(params$year) -4) %>%
  ...
  labs(
    ...
    title = paste("Price / Price of", year_minus_3, ", Outliers Excluded"),
  ) +
  theme_minimal() +
  theme(
    legend.position = "none",
