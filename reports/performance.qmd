...
suppressPackageStartupMessages(
  purrr::walk(
    strsplit(yaml::read_yaml(here("DESCRIPTION"))$`Config/renv/profiles/reporting/dependencies`, ", ")[[1]],
    library,
    character.only = TRUE
  )
)
...
res_chars <- read_parquet(paths$input$char$local) %>%
...
chars_data %>%
  inner_join(hie_data %>% select(-year), by = join_by(meta_pin == pin)) %>%
  left_join(
