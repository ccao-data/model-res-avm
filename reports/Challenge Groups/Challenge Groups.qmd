...
Within the universe of assessed properties, there are characteristics which make it difficult to predict their market value. These properties are separated out and analyzed in the following report. All outputs are calculated on county-level data. Data for sold properties is calculated from sales in the prior year.
...
**Missing Critical Value:** Indicates a Pin where one of the following variables are missing (meta_year, meta_pin, char_yrblt, char_bldg_sf, char_land_sf, char_beds, char_fbath, char_apts)
...
bind_rows(
  ...
  full_data_functions(critical_values_missing, "Missing Critical Values")
) %>%
...
top_decile_sold <- top_decile %>% 
  filter(!is.na(sale_ratio_study_price))
critical_values_missing_sold <- critical_values_missing %>%
  filter(!is.na(sale_ratio_study_price))
...
datasets_sold <- list(
  ...
  "Critical Values Missing" = critical_values_missing_sold
)
...
cbind(results1, results2) %>%
  ...
  select(dataset, count, 'median ratio' = median_ratio, cod, prb, prd, mki, min, q25, median, 'median previous estimate' = median_prior_near_tot, q75, max, rmse, 'r squared' = r_squared, mae, mpe, mape, mdape) %>%
  kable(format = "html", digits = 2, row.names = FALSE) %>%  
  kable_styling()
...
plots <- imap(datasets_sold, ~ {
  p <- ggplot(.x, aes(x = sale_ratio_study_price / 1000000, y = pred_pin_initial_fmv / 1000000, 
                      text = paste("Pin:", meta_pin, 
                                   "<br>Township:", ccao::town_convert(meta_township_code), 
                                   "<br>Sale Price:", scales::dollar(sale_ratio_study_price), 
                                   "<br>Estimated Price:", scales::dollar(pred_pin_initial_fmv)))) + 
    geom_point() +
    geom_abline(intercept = 0, slope = 1, color = "red") + 
    scale_x_continuous(
      name = "Actual FMV (Sale Price in $Million)",
      limits = c(0, 2)
    ) + 
    scale_y_continuous(
      name = "Estimated FMV (in $Million)",
      limits = c(0, 2)  
    ) +
...
process_and_plot <- function(data) {
  ...
  # Create the plot
  ggplot() +
    geom_sf(data = spatial_data, aes(color = Increase)) +  
    geom_sf(data = town_shp, fill = NA, color = "black") + 
    theme_minimal() +
    labs(color = "Increase (%)") +
    scale_color_continuous(breaks = c(0, 100, 200, 300), limits = c(0, 300)) +  # Ensure consistent color scales
