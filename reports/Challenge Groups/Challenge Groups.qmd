...
bind_rows(
  ...
  full_data_functions(critical_values_missing, "Critical Values Missing")
) %>%
...
plots <- imap(datasets_sold, ~ {
  ...
  p <- ggplot(.x, aes(x = sale_ratio_study_price / 1000, y = pred_pin_initial_fmv / 1000
...
process_and_plot <- function(data) {
  ...
  ggplot() +
    ...
    theme_minimal() +
    labs(color = "Change (%)") +
    scale_color_gradient2(midpoint = 0, low = "blue", mid = "lightgrey", high = "red", limits = c(-100, 300)) + # Use gradient for coloring
    theme(
