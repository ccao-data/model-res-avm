...
outliers_detect <- function(x) {
  ...
  x < median - 6 * mad | x > median + 6 * mad
  
}

top5_outliers <- shaps %>%
  mutate(triad = ccao::town_get_triad(township_code)) %>%
  select(meta_pin, triad, run_id, !!top5) %>%
  group_by(triad, run_id) %>%
...
temp <- lapply(top5, function(x) {
  ...
  top5_outliers %>% 
    select(contains(x), triad, run_id) %>%
    group_by(triad, run_id) %>%
    mutate(median = median(.data[[x]], na.rm = TRUE)) %>%
    ungroup() %>%
    filter(.[[2]]) %>%
    mutate(up_low = case_when(.data[[x]] > median ~ "upper", TRUE ~ "lower"))
...
hug <- mapply(function(data, model) {
      ...
      addLegend("bottomright", pal = pal, values = ~ up_low,
        title = paste0("Shap Outliers</br>", names(data)[1], "</br>", model),
        opacity = 1
      )
      
  
}, rep(temp, 2), c(rep(model_ids[1], length(temp)), rep(model_ids[2], length(temp))), SIMPLIFY = FALSE, USE.NAMES = TRUE)
...
::: {.column width="44%"}
```{r}
tagList(hug[(length(hug)/2+1):length(hug)])
```
:::
