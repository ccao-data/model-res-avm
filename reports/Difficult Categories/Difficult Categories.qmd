...
apply_functions <- function(dataset, dataset_name) {
  ...
  median_estimate <- median(dataset$pred_pin_initial_fmv, na.rm = TRUE)
...
table <- cbind(results1, results2) %>%
  as.data.frame() %>%
  mutate(median_ratio = median_estimate / median_prior_near_tot) %>%
  select(dataset, median_ratio, cod, prb, prd, mki, min, q25, median, q75, max, rmse, r_squared, mae, mpe, mape, mdape, median_prior_near_tot, median_estimate) %>%
