...
Cook County is an extremely large and diverse housing market. It spans millions of properties that vary widely in type, age, location, and quality. Accurately estimating the price of such different properties is a complicated, challenging task. 
...
More traditionally, we use R<sup>2</sup>, root-mean-squared-error (RMSE), and mean absolute error (MAE) to gauge overall model performance and fit. Overall model performance on the [test set](#data-used) as of `r Sys.Date()` is shown in the table below and generally stays within this range.
...
6. Run the `model.R` script. This trains the model, evaluates a test set, and generates a report on the model's performance
