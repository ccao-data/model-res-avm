...
About half the tracts have an ARI of 0. This was pretty poorly demonstrated on their maps, where they had a category of 1 or less, but in reality it refers to a lack of data. Because of this, I took it out of the future maps, meaning that in some maps, there are a significant number of empty tracts.
...
### Median Absolute SHAP Values by Tract, Keeping Tracts with 0 ARI
```{r, echo = FALSE}
...
Looking at the data on a less granular level, we can see that the SHAP values are higher in the North side of the County. This likely relates to the fact that the North side of the County is more expensive, and the model looks for reasons to identify higher prices. 

```{r, echo = FALSE}
ggplot(data = tract_data_non_zero) +
  geom_sf(aes(fill = shap_township_mean)) +
  scale_fill_viridis_c(option = "viridis") +
  theme_void() +
  labs(
    fill = "Added $ Value"
