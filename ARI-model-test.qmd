...
### Median Absolute SHAP Values by Tract,  Tracts with 0 ARI

Some of the same patterns are visible when we look at the data without the tracts with 0 ARI. But, the map is generally more complicated to interpret. 
```{r, echo = FALSE}
...
Looking at the data on a less granular level, we can see that the SHAP values are higher in the North side of the County. This reflects the findings on the tract leve. 
