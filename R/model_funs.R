...
predict.stack_model <- function(object, new_data) {
  ...
  cknn_preds <- unname(unlist(cknn_predict(
  ...
  new_preds <- pmap(
    ...
    ~ model_predict(.x, .y, new_data %>% mutate(cknn_preds = cknn_preds))
  )
  ...
  c(cknn = list(cknn_preds), new_preds, stack = list(stack_preds$.pred))
}
