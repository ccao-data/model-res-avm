...
# Calculate the number of sales within the last three years. This function works
...
num_sales_prev_n_years <- function(
    ...
    years,
    set = c("training", "assessment")) {
  if (set == "training") {
    # Join all sales for a PIN to each sale for that PIN
    output <- left_join(
      training_data,
      inner_join(
        training_data %>%
          select(meta_pin, meta_sale_document_num, meta_sale_date),
        training_data %>% select(meta_pin, meta_sale_date),
        by = "meta_pin",
        relationship = "many-to-many"
      ) %>%
        # as.duration(1) excludes the same sale from being identified as within
        # 3 years of itself
        mutate(within_three_years = between(
    ...
    return(output)
  } else if (set == "assessment") {
    # Join sales from training set to assessment set
    output <- left_join(
      assessment_data,
      left_join(
        assessment_data %>% select(meta_pin),
        training_data %>% select(meta_pin, meta_sale_document_num, year),
        by = "meta_pin",
        relationship = "many-to-many"
      ) %>%
        distinct() %>%
