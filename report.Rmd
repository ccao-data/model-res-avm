...
test_summary <- test_res %>%
  ...
  summarize(
    ...
    final = ccao_cod(stack / meta_sale_price)$COD,
    final_prd = ccao_prd(stack, meta_sale_price)$PRD,
    final_prb = ccao_prb(stack, meta_sale_price)$PRB,
  ) %>%
  mutate(
    final_cod_met = cod_met(final),
    final_prd_met = prd_met(final_prd),
    final_prb_met = prb_met(final_prb)
  ) %>%
  arrange(final) %>%
...
headers <- htmltools::withTags(table(
  ...
  thead(
    tr(
      ...
      th(rowspan = 2, "Count"),
      th(colspan = 5, "Model CODs"),
      th(colspan = 4, "Final Model Stats")
    ),
    tr(
      th("enet"), th("lgbm"), th("cat"), th("rf"), th("stack"),
      th("COD"), th("PRD"), th("PRB")
    )
  )
))
...
test_summary %>%
  datatable(
    ...
    options = list(
      ...
      searching = FALSE,
      info = FALSE,
      columnDefs = list(list(targets = 11:13, visible = FALSE))
    )
  ) %>%
...
model_order <- c("enet", "lgbm", "cat", "rf", "stack", "final")
...
param_headers <- htmltools::withTags(table(
  ...
  thead(
    tr(
      th(rowspan = 2, "Model"),
      th(rowspan = 2, "Iterations"),
      th(colspan = 9, "Parameters"),
    ),
    tr(
      th("penalty"), th("mixture"), th("mtry"), th("trees"), th("tree_depth"),
      th("min_n"), th("learn_rate"), th("loss_reduction"), th("sample_size")
    )
  )
))
...
model_results %>%
  ...
  formatRound(c(3:4, 9:11), digits = 4)
...
if (exists("enet_params")) {
  autoplot(enet_params, metric = "rmse", type = "marginals") +
    geom_smooth(se = F) +
...
if (exists("rf_params")) {
  autoplot(rf_params, metric = "rmse", type = "marginals") +
    geom_smooth(se = F) +
...
if (file.exists(model_timings_path)) {
  readRDS(model_timings_path) %>%
    ...
    mutate(
      ...
      model = factor(model, levels = c("elasticnet", "lgbm", "rf"))
    ) %>%
  ggplot() +
