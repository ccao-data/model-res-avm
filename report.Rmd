...
test_summary <- test_res %>%
  ...
  summarize(
    ...
    `Final PRD` = ccao_prd(stack, meta_sale_price)$PRD,
    `Final PRB` = ccao_prb(stack, meta_sale_price)$PRB
  ) %>%
  arrange(final) %>%
...
test_summary %>%
  rename(Township = town_name, `Num. Trimmed` = num_trimmed, Final = final) %>%
  rename_with(~ paste(.x, "COD"), .cols = c(enet:stack, Final)) %>%
  datatable(
    rownames = FALSE,
    filter = "none",
