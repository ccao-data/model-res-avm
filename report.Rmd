...
test_summary <- test_res %>%
  ...
  summarize(
    count = n(),
    across(cknn:stack, ~ cod(.x / meta_sale_price), .names = "{.col}_cod"),
...
model_order <- c("cknn", "enet", "xgb", "rf", "stack", "stack_trim")

# Generate township level map of COD
test_summary %>%
  pivot_longer(c(cknn:stack, stack_trim)) %>%
...
decile_data <- test_res %>%
  mutate(
    ...
    across(cknn:stack, ~ .x / meta_sale_price)
  ) %>%
  group_by(decile) %>%
  ...
  pivot_longer(cknn:stack, names_to = "model", values_to = "ratio") %>%
...
model_cor <- as_tibble(cor(select(test_res, cknn:stack), method = "spearman")) %>%
  mutate(name2 = names(.)) %>%
  pivot_longer(-name2) %>%
  mutate(
