...
All inputs and outputs are stored on AWS S3 via a unique run identifier.
Each stage in the modeling pipeline corresponds to an individual R
script. These scripts can be run independently (as a stand-alone script)
or as part of the overall pipeline (with [DVC](#using-dvc)) as long as
the dependencies for the stage exist.
...
4.  **Interpret**: Calculate SHAP values for all the estimated values
    ...
    single PIN). The primary output of this stage is a data frame of the
    contributions of each feature for each property.
...
6.  **Export**: Export assessed values to Desk Review spreadsheets for
    Valuations, as well as a delimited text format for upload to
    iasWorld. NOTE: This stage is only run when a final model is
    selected. It is not run automatically or as part of the main
    pipeline.
...
framework created and maintained by Microsoft. It was only recently
...
|:--------------------------------------------------------------------------------------------------------------------------|:----------------|:--------------------------------------------------------------------------------------------------------------------------------|
| [stop\_iter](https://ccao-data-science---modeling.gitlab.io/packages/lightsnip/reference/train_lightgbm.html)             | 10 - 40         | Stop training after number of iterations without improvement on a validation set.                                               |
| [num\_leaves](https://lightgbm.readthedocs.io/en/latest/Parameters.html#num_leaves)                                       | 500 - 2000      | Maximum number of leaves in each tree. Main parameter to control model complexity.                                              |
| [add\_to\_linked\_depth](https://ccao-data-science---modeling.gitlab.io/packages/lightsnip/reference/train_lightgbm.html) | 1 - 3           | Amount to add to `max_depth` if linked to `num_leaves`. See `max_depth`.                                                        |
| [feature\_fraction](https://lightgbm.readthedocs.io/en/latest/Parameters.html#feature_fraction)                           | 0.3 - 0.9       | The random subset of features selected for a tree, as a percentage.                                                             |
...
| [min\_data\_in\_leaf](https://lightgbm.readthedocs.io/en/latest/Parameters.html#min_data_in_leaf)                         | 2 - 200         | The minimum data in a single tree leaf. Important to prevent over-fitting.                                                      |
...
| [min\_data\_per\_group](https://lightgbm.readthedocs.io/en/latest/Parameters.html#min_data_per_group)                     | 20 - 250        | Minimum number of observations per categorical group                                                                            |
...
| [cat\_l2](https://lightgbm.readthedocs.io/en/latest/Parameters.html#cat_l2)                                               | 0.001 - 100     | Categorical-specific L2 regularization                                                                                          |
| [lambda\_l1](https://lightgbm.readthedocs.io/en/latest/Parameters.html#lambda_l1)                                         | 0.001 - 100     | L1 regularization                                                                                                               |
| [lambda\_l2](https://lightgbm.readthedocs.io/en/latest/Parameters.html#lambda_l2)                                         | 0.001 - 100     | L2 regularization                                                                                                               |
| [max\_depth](https://lightgbm.readthedocs.io/en/latest/Parameters.html#max_depth)                                         | 4 - 16          | Maximum tree depth of the model. Usually linked to `num_leaves` via `max_depth = floor(log2(num_leaves)) + add_to_linked_depth` |
| [max\_bin](https://lightgbm.readthedocs.io/en/latest/Parameters.html#max_bin)                                             | 200 - 1500      | Maximum number of bins used to bucket continuous features                                                                       |
...
sales](#data-used) in our training sample.
...
| Walkability                                       | The \[Chicago Metropolitan Agency for Planning’s\] ON TO 2050 [Walkability Scores](https://datahub.cmap.illinois.gov/dataset/on-to-2050-layer-walkability#:~:text=While%20several%20measures%20of%20walkability,comfortable%2C%20and%20the%20walk%20is)                                                  |
...
is always the period immediately following the training set. This helps
cross-validation determine which hyperparameters will perform best when
...
peforms a set of simple adjustments on the predicted values from
`assessment_data`. These adjustments are internally called
...
1.  Aggregate values for multi-card properties to the PIN level, then
    ...
    prior year values. This is needed back often back buildings (ADUs,
    secondary buildings) will receive a much higher initial value than
