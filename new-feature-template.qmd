...
Before running this report, make sure that the lightgbm model is set to optimize the Root Mean Square Error (RMSE). This means that the addition of a new variable *should* improve this metric. In this situation, the model sees a change of `r output$rmse` to `r output$rmse_new`, a change of `r output$rmse_new - output$rmse`.
...
neighborhood <- datatable_list[[1]]
township <- datatable_list[[2]]
triad <- datatable_list[[3]]
...
```{r, results = 'asis'}
iwalk(datatable_list, function(dt, geo_type) {
  cat("##", geo_type, "\n\n")
  print(dt)
  cat("\n\n")
})
...
lapply(township_codes, function(code) {
...
### Neighborhoods with lowest Mean Value
...
### Neighborhoods with the highest relative SHAP values
...
### Neighborhoods with the lowest relative SHAP values
...
### 10 Lowest values per neighborhood
