...
generate_comparison_table <- function(df_new, df_comparison, title, group_var) {
  df_new <- df_new %>%
...
```{r}
generate_comparison_table(df_stat_no_group, df_stat_no_group_comparison, "Overall Statistics Comparison", NULL)
```
...
overall_medians <- shap_df_filtered_long %>%
  group_by(feature) %>%
  summarize(median_abs_shap = median(abs(shap), na.rm = TRUE),
...
create_summary_table <- function(data, added_variable, title) {
...
overall_summary_table <- create_summary_table(ranked_overall_medians, added_variable, "Summary for All Data")

# Calculate median values for each township and rank features
shap_df_filtered_long <- shap_df_filtered_long %>%
  group_by(township_code, feature) %>%
  mutate(median_abs_shap = median(abs(shap), na.rm = TRUE),
         median_shap = median(shap, na.rm = TRUE)) %>%
  ungroup() %>%
...
```{r}
assessment_pin %>%
  select(meta_pin, loc_longitude, loc_latitude, meta_nbhd_code, pred_pin_final_fmv) %>%
  ...
  group_by(meta_nbhd_code) %>%
  mutate(median_fmv_new = median(pred_pin_final_fmv_new),
...
```{r}
largest_fmv_increases <- assessment_pin %>%
  select(meta_pin, loc_longitude, loc_latitude, pred_pin_final_fmv) %>%
  rename(pred_pin_final_fmv_new = pred_pin_final_fmv) %>%
  inner_join(assessment_pin_comparison, by = "meta_pin") %>%
  rename(pred_pin_final_fmv_comparison = pred_pin_final_fmv) %>%
  mutate(fmv_change = pred_pin_final_fmv_new - pred_pin_final_fmv_comparison) %>%
  arrange(desc(fmv_change)) %>%
  slice(1:100)

pal_largest_fmv_increases <- colorNumeric(palette = "viridis", domain = largest_fmv_increases$fmv_change)
...
leaflet(largest_fmv_increases) %>%
  ...
  addCircleMarkers(
    ...
    lat = ~loc_latitude,
    radius = 5,
    color = ~pal_largest_fmv_increases(fmv_change),
    popup = ~paste(
      "Pin: ", meta_pin,
      "<br>", "FMV New: ", sprintf("%.2f", pred_pin_final_fmv_new),
      "<br>", "FMV Comparison: ", sprintf("%.2f", pred_pin_final_fmv_comparison),
      "<br>", "FMV Change: ", sprintf("%.2f", fmv_change)
    )
  ) %>%
  addLegend(
    "bottomright",
...
```{r}
largest_fmv_decreases <- assessment_pin %>%
  select(meta_pin, loc_longitude, loc_latitude, pred_pin_final_fmv) %>%
  rename(pred_pin_final_fmv_new = pred_pin_final_fmv) %>%
  inner_join(assessment_pin_comparison, by = "meta_pin") %>%
  rename(pred_pin_final_fmv_comparison = pred_pin_final_fmv) %>%
...
pal_largest_fmv_decreases <- colorNumeric(palette = "viridis", domain = largest_fmv_decreases$fmv_change)
...
leaflet(largest_fmv_decreases) %>%
  ...
  addCircleMarkers(
    ...
    lat = ~loc_latitude,
    radius = 5,
    color = ~pal_largest_fmv_decreases(fmv_change),
    popup = ~paste(
      "Pin: ", meta_pin,
      "<br>", "FMV New: ", sprintf("%.2f", pred_pin_final_fmv_new),
      "<br>", "FMV Comparison: ", sprintf("%.2f", pred_pin_final_fmv_comparison),
...
Feature selection for the lightgbm model is not as straightforward as selecting the statistical significant variables, and because for interpretability, the inclusion of all variables in their raw form is necessary, we cannot use dimension reduction techniques. In general, the lighgbm model does a good job of selecting the most important variables, so 

To remove the new feature from the model, it should
- Not improve model metrics
- Be one of the lowest SHAP value indicators
- Not have profound effects in particular neighborhoods 
