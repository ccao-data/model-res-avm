...
assessment_pin <- read_parquet(
  "/home/damajor/model-res-avm/output/assessment_pin/model_assessment_pin.parquet") %>%
  mutate(township_name = ccao::town_convert(meta_township_code))

assessment_card <- read_parquet(
  "/home/damajor/model-res-avm/output/assessment_card/model_assessment_card.parquet")
```
...
Previous iterations of this test demonstrated that the `training data` has implausible (60 bedrooms) or impossible values (-2 bathrooms). Because of this, we implemented filtering in the ingest stage. The rendered result reflects these changes.
...
assessment_pin %>%
  ...
  select(meta_pin, meta_class, pred_pin_final_fmv, township_name, char_total_bldg_sf, char_yrblt) %>%
  datatable(options = list(scrollY = "400px", paging = FALSE))
```
...
Here we look at three types of sales which may be outliers. The first is the most expensive 20 sales in each township. The second is the most expensive 20 sales in each township based on price per square foot. The third is the least expensive 20 sales in each township based on price per square foot.
