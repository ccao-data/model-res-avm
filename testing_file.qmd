...
training_data %>%
  ...
  slice(1:100) %>%
  select(meta_pin, meta_class, char_bldg_sf) %>%
  datatable(options = list(scrollY = '400px', paging = FALSE))
```
...
subset <- bind_rows(subset_top_sales, subset_highest_ratios, subset_lowest_ratios) %>%
  select(township_name, meta_pin, sale_recent_1_price, char_bldg_sf, price_per_sq_ft, type, pred_pin_final_fmv_round)

scatterplot <- subset %>%
  ggplot(aes(x = sale_recent_1_price, y = pred_pin_final_fmv_round, color = type, text = paste(
...
interactive_scatterplot <- ggplotly(scatterplot, tooltip = "text") %>%
  layout(
    xaxis = list(range = c(0, 10000000)),
    yaxis = list(range = c(0, 10000000))
  )
