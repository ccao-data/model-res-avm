...
working_data_card <- shap %>%
  ...
  ungroup() %>%
  group_by(meta_nbhd_code, meta_pin) %>%
  mutate(
...
temp_plurality <- working_data_card %>%
...
working_data_card <- working_data_card %>%
  left_join(temp_plurality, by = "meta_nbhd_code")
...
create_leaflet_map(bottom_10_data_relative, params$added_feature_shap, "Relative SHAP Values", order_scheme = "low")
```
