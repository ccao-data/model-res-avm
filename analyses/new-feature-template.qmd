...
metadata_comparison <- fetch_analyses(params$comparison_run_id, params$comparison_run_id_year, comp_paths)
...
ranked_overall_medians <- shap_df_filtered_long %>%
  group_by(feature) %>%
  summarize(
...
ranked_overall_medians_township <- shap_df_filtered_long %>%
  group_by(township_code, feature) %>% # Group by township and feature
  summarize(
    median_abs_shap = median(abs(shap), na.rm = TRUE),
    median_shap = median(shap, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  group_by(township_code) %>% # Group by township code
  arrange(desc(median_abs_shap), .by_group = TRUE) %>% # Arrange within each township
  mutate(rank_abs = row_number()) %>% # Create rank within each township
  ungroup()
...
overall_summary_table_township <- create_summary_table(ranked_overall_medians_township, {{ added_feature }})
