...
shap_comparison %>%
  ...
  mutate(across(ends_with(".y"), ~ . - get(sub("\\.y$", ".x", cur_column())), .names = "{sub('.y$', '_diff', .col)}")) %>%
  select(-ends_with(".x"), -ends_with(".y")) %>%
  summarise(across(ends_with("_diff"), list(
    median_change = ~ median(.),
    median_abs_change = ~ median(abs(.))
  ))) %>%
  pivot_longer(cols = everything(), names_to = c("variable", ".value"), names_sep = "_diff_") %>%
  arrange(variable)
```
