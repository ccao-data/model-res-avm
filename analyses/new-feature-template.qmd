...
params:
  ...
  added_variable: "ccao_is_corner_lot"
  added_variable_shap: "ccao_is_corner_lot_shap"
  min_range: 5
  max_range: 95
  type: "categorical"
---
...
assessment_data_small <- assessment_data %>%
  select(meta_pin, meta_card_num, meta_nbhd_code, loc_longitude, loc_latitude, meta_township_name, !!sym(params$added_variable))
...
working_data_card <- shap %>%
  ...
  mutate(
    shap_relative = percent((!!sym(params$added_variable_shap) / pred_pin_initial_fmv_new), accuracy = 0.01),
...
if (params$type == "continuous") {
  ...
  category_percentages_township <- working_data_pin %>%
    count(meta_township_name, !!sym(params$added_variable)) %>%
    mutate(percentage = round(n / sum(n) * 100, 2)) %>%
    select(meta_township_name, !!sym(params$added_variable), percentage) %>%
    pivot_wider(names_from = !!sym(params$added_variable), values_from = percentage, values_fill = list(percentage = 0))
...
if (params$type == "continuous") {
  ...
  category_percentages_township <- working_data_pin %>%
    group_by(meta_township_name, !!sym(params$added_variable)) %>%
    count() %>%
    group_by(meta_township_name) %>%
    mutate(percentage = round(n / sum(n) * 100, 2)) %>%
    select(meta_township_name, !!sym(params$added_variable), percentage) %>%
...
if (params$type == "continuous") {
  ...
  category_percentages_nbhd <- working_data_pin %>%
    group_by(meta_nbhd_code, !!sym(params$added_variable)) %>%
    count() %>%
    mutate(percentage = n / sum(n) * 100) %>%
    select(meta_nbhd_code, !!sym(params$added_variable), percentage)
...
if (params$type == "continuous") {
  ...
  ggplot(working_data_pin, aes(x = !!sym(params$added_variable))) +
    ...
    theme_minimal() +
    annotate("text", x = descriptives$mean, y = Inf, label = "Mean", color = "red", vjust = 1.5, hjust = -0.5) +
    annotate("text", x = descriptives$median, y = Inf, label = "Median", color = "green", vjust = 1.5, hjust = 0.5)
} else if (params$type == "categorical") {
  category_percentages <- working_data_pin %>%
    count(!!sym(params$added_variable)) %>%
    mutate(percentage = n / sum(n) * 100)

  ggplot(category_percentages, aes(x = !!sym(params$added_variable), y = percentage)) +
    geom_bar(stat = "identity", fill = "blue", color = "black", alpha = 0.7) +
    labs(
      title = paste("Bar Chart of", params$added_variable),
      x = params$added_variable,
      y = "Percentage"
    ) +
    theme_minimal()
...
ggplot(
  ...
  theme_minimal() +
  annotate("text", x = mean(working_data_pin$diff_pred_pin_initial_fmv, na.rm = TRUE), y = Inf, label = "Mean", color = "red", vjust = 1.5, hjust = -1) +
  annotate("text", x = median(working_data_pin$diff_pred_pin_initial_fmv, na.rm = TRUE), y = Inf, label = "Median", color = "green", vjust = 1.5, hjust = 1)
```
...
ggplot(
  ...
  theme_minimal() +
  annotate("text", x = mean(shap[[params$added_variable]], na.rm = TRUE), y = Inf, label = "Mean", color = "red", vjust = 1.5, hjust = -1) +
  annotate("text", x = median(shap[[params$added_variable]], na.rm = TRUE), y = Inf, label = "Median", color = "green", vjust = 1.5, hjust = 1)
```
...
Here, the goal is to see if the added variable *very* neatly aligns with other existing variables. Columns are produced with both the absolute value of the correlation (for easy sorting), as well as the correlation to help decipher the direction of the relationship.
...
# Gather the data for the top 20 variables
top_vars <- top_20_correlations$Column
working_data %>%
  select(!!sym(params$added_variable), all_of(top_vars)) %>%
  gather(key = "var", value = "value", -!!sym(params$added_variable)) %>%
  ggplot(aes(x = !!sym(params$added_variable), y = value)) +
...
Below are other variables, organized by different geographies to see if the the added variable improves other metrics that the office is interested in. 
...
Here, we compare the SHAP values from the comparison data and the new data. The goal here is to see which other variables, the added variable is influencing.
```{r shap_comparisons}
shap_comparison %>%
  ...
  pivot_longer(cols = everything(), names_to = c("variable", ".value"), names_sep = "_diff_|_comp_|_new_") %>%
  arrange(variable) %>%
...
The assumption is, that if the added variable caused an important increase / decrease in assessed values, then the correlation would have a high absolute value. In this case, the correlation between the SHAP values and the added variable is `r round(correlation_value, 2)`.


# Spatial Analysis

The spatial analysis is broken up into a few sections. The first panel looks at four aggregated stats on the neighborhood level, the mean of the variable we are analyzing, the mean of the absolute value of the SHAP, the 90th percentile of the SHAP, and the change in FMV based on the added variable. 
...
if (params$type == "categorical") {
  spatial_data %>%
    ggplot() +
    geom_sf(aes(fill = !!sym(paste0(params$added_variable, "_plurality_type")))) +
    scale_fill_viridis_d(option = "viridis", name = "Type of Plurality") +
    geom_sf(aes(alpha = !!sym(paste0(params$added_variable, "_plurality_percentage")))) +
  ...
  leaflet(dataset) %>%
    ...
    addCircleMarkers(
      ...
      popup = ~ paste(
        ...
        "<br>", "SHAP:", scales::dollar(dataset[[params$added_variable_shap]]),
        ...
        "<br>", "FMV Difference: ", scales::dollar(dataset$diff_pred_pin_final_fmv)
      )
    ) %>%
    addLegend(
      "bottomright",
