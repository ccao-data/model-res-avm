...
When adding a nwe variable, some metrics may be improved only within certain ranges. For example, proximity to an ammenity may only matter within 100 feet. Thus, we filter the data to include a minimum of `r params$min_range` and a maximum of `r params$max_range`.
...
generate_comparison_table(new_range, comparison_range, "Range Test")
```
...
create_leaflet_map <- function(dataset, legend_value, legend_title, longitude = "loc_longitude", latitude = "loc_latitude") {
  ...
  pal <- colorNumeric(palette = "viridis", domain = dataset[[legend_value]])
