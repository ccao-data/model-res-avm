...
```{r shap_township_importance, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='asis',fig.keep='all'}
shap_df_filtered_long %>%
...
```{r scatterplot, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='asis',fig.keep='all'}
shapviz::shapviz(
  object = shap_new %>%
...
## Violin Plot
```{r violin_plot, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='asis',fig.keep='all'}
...
if (!is.numeric(card_individual %>% pull({{ target_feature_value }}))) {
  # Convert to factor and manually group top categories
  card_individual <- card_individual %>%
    mutate(bin = forcats::fct_lump_n(!!sym(target_feature_value), n = 5))
} else {
  # Bin numerically if the column is numeric
  card_individual <- card_individual %>%
    mutate(bin = cut_number(!!sym(target_feature_value), n = 5, dig.lab = num_digits))
}

# Calculate quantiles
quantiles <- quantile(card_individual %>% pull({{ target_feature_shap }}), c(0.025, 0.975), na.rm = TRUE)
...
card_individual %>%
  ...
  theme_minimal() +
  xlab("Feature Value (Binned or Categorized)") +
  ylab("SHAP Value") +
  ...
  ggtitle("Violin Plot of SHAP Values for Top 5 Largest Features")
```
:::
