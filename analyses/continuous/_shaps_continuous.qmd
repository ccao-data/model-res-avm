...
num_bins <- min(5, n_distinct(card_individual %>% pull(!!sym(target_feature_value))))

# Add bins to the dataset
card_individual <- card_individual %>%
  mutate(
    bin = if (n_distinct(!!sym(target_feature_value)) <= 5) {
      as.character(!!sym(target_feature_value))
    } else {
      cut_number(!!sym(target_feature_value), n = num_bins, dig.lab = 2)
    }
  )
...
card_individual %>%
  ...
  ggtitle("Violin Plot of SHAP Values for Top 5 Largest Feature Ranges")
