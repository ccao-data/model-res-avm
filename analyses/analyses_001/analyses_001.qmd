...
assessment_pin %>%
  ...
  slice(1:10) %>%
  ungroup() %>%
  mutate(fmv = scales::dollar(pred_pin_final_fmv)) %>%
  select(
    meta_pin,
    meta_class,
...
plots <- assessment_pin %>%
  ...
  map(~ ggplot(.x, aes(x = pred_pin_initial_fmv)) +
    ...
    labs(
      title = paste("Distribution of Initial FMV for",
                    unique(.x$township_name)),
      x = "Initial FMV",
      y = "Frequency"
    ) +
    theme_minimal() +
...
subset <- bind_rows(subset_top_sales,
...
scatterplot <- subset %>%
  ggplot(aes(
    ...
    color = type, text = paste(
      ...
      "<br>Difference: $",
      scales::comma(pred_pin_final_fmv_round - sale_recent_1_price),
