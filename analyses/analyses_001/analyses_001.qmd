...
##  Largest `char_bldg_sf` Properties Per Township
...
# Distribution of `pred_pin_initial_fmv`
```{r}
...
# Distribution of `meta_sale_price`
```{r}
plots <- training_data %>%
  filter(meta_triad_code == "2") %>%
  group_by(township_name) %>%
  group_split() %>%
  set_names(map_chr(., ~ unique(.x$township_name))) %>%
  map(~ ggplot(.x, aes(x = meta_sale_price)) +
    geom_histogram(binwidth = 50000, fill = "skyblue", color = "black") +
    labs(
      title = paste("Distribution of Sale Price for",
                    unique(.x$township_name)),
      x = "meta_sale_price",
      y = "Frequency"
    ) +
