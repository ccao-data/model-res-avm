...
```{r _spatial_analysis_continuous_mean_feature_neighborhood}
pin_nbhd %>%
  ggplot() +
...
```{r _spatial_analysis_continuous_median_feature_neighborhood}
pin_nbhd %>%
  ggplot() +
...
```{r _spatial_analysis_continuous_mode_feature_neighborhood}
pin_nbhd %>%
  ggplot() +
...
```{r _spatial_analysis_continuous_mean_shap_neighborhood}
card_nbhd %>%
  ggplot() +
...
```{r _spatial_analysis_continuous_90th_percentile_shap}
card_nbhd %>%
  ggplot() +
...
```{r _spatial_analysis_continuous_neighborhood_change}
assessment_pin_new %>%
...
```{r, _spatial_analysis_continuous_100_lowest}
...
```{r, _spatial_analysis_continuous_100_highest_shap}
highest_100 <- leaflet_data %>%
...
```{r, _spatial_analysis_continuous_100_lowest_shap}
lowest_100 <- leaflet_data %>%
  arrange(shap_total) %>%
  slice(1:100)
...
```{r, _spatial_analysis_continuous_100_largest_fmv_increases}
...
```{r, _spatial_analysis_continuous_100_largest_fmv_decreases}}
...
```{r, _spatial_analysis_continuous_100_largest_fmv_initial_increases}
largest_fmv_increases <- leaflet_data %>%
...
```{r, _spatial_analysis_continuous_100_largest_fmv_initial_decreases}}
largest_fmv_decreases <- leaflet_data %>%
...
```{r, _spatial_analysis_continuous_100_largest_fmv_increases_no_multicards}}
largest_fmv_increases <- leaflet_data %>%
  group_by(meta_pin) %>%
  filter(n() == 1) %>%
  ungroup() %>%
...
```{r, _spatial_analysis_continuous_100_largest_fmv_decreases_no_multicards}}}
largest_fmv_decreases <- leaflet_data %>%
  group_by(meta_pin) %>%
  filter(n() == 1) %>%
  ungroup() %>%
...
::: panel-tabset
```{r _spatial_analysis_continuous_processing_SHAP_values}
...
```{r, _spatial_analysis_continuous_2_highest_shap_nbhd}
create_leaflet_map(high_mean_data, "shap_total", "SHAP Values")
```

### 2 Lowest SHAP Neighborhoods

```{r, _spatial_analysis_continuous_2_lowest_shap_nbhd}
