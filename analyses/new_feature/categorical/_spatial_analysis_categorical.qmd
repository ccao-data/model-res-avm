...
```{r _spatial_analysis_categorical_mean_shap_neighborhood}
card_nbhd %>%
  ggplot() +
...
```{r _spatial_analysis_categorical_90th_percentile_shap}
card_nbhd %>%
  ggplot() +
...
```{r _spatial_analysis_categorical_neighborhood_change}
assessment_pin_new %>%
...
```{r, _spatial_analysis_categorical_100_highest_shap}
...
```{r, _spatial_analysis_categorical_100_lowest_shap}
...
```{r, _spatial_analysis_categorical_100_largest_fmv_increases}
...
```{r, _spatial_analysis_categorical_100_largest_fmv_decreases}}
...
```{r, _spatial_analysis_categorical_100_largest_fmv_initial_increases}
largest_fmv_increases <- leaflet_data %>%
...
```{r, _spatial_analysis_categorical_100_largest_fmv_initial_decreases}}
largest_fmv_decreases <- leaflet_data %>%
...
```{r, _spatial_analysis_categorical_100_largest_fmv_increases_no_multicards}}
largest_fmv_increases <- leaflet_data %>%
  group_by(meta_pin) %>%
  filter(n() == 1) %>%
  ungroup() %>%
...
```{r, _spatial_analysis_categorical_100_largest_fmv_decreases_no_multicards}}}
largest_fmv_decreases <- leaflet_data %>%
  group_by(meta_pin) %>%
  filter(n() == 1) %>%
  ungroup() %>%
...
create_leaflet_map(largest_fmv_increases, "diff_pred_pin_final_fmv", "Largest FMV Decreases (%)", order_scheme = "low", display_as_percent = TRUE)
```
...
```{r _spatial_analysis_categorical_processing_SHAP_values}
...
```{r, _spatial_analysis_categorical_2_highest_shap_nbhd}
create_leaflet_map(high_mean_data, "shap_total", "SHAP Values")
```

### 2 Lowest SHAP Neighborhoods

```{r, _spatial_analysis_categorical_2_lowest_shap_nbhd}
