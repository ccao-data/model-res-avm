...
```{r _data_processing_categorical}
...
pin_nbhd <- pin_individual %>%
  ...
  pivot_wider(
    names_from = rank,
    values_from = c(percentage, !!sym(target_feature_value)),
    names_glue = "{.value}_mode_{rank}"
  ) %>%
  group_by(meta_nbhd_code) %>%
  summarise(
    primary_mode_percentage = ifelse(is.infinite(max(percentage_mode_1, na.rm = TRUE)), NA, max(percentage_mode_1, na.rm = TRUE)),
    secondary_mode_percentage = ifelse(is.infinite(max(percentage_mode_2, na.rm = TRUE)), NA, max(percentage_mode_2, na.rm = TRUE)),
    primary_mode = first(na.omit(!!sym(paste0(target_feature_value, "_mode_1")))),
    secondary_mode = first(na.omit(!!sym(paste0(target_feature_value, "_mode_2"))))
  ) %>%
