...
```{r shap_township_importance, include = FALSE}
shaps_township <- shap_df_filtered_long %>%
...
## Violin Plots of SHAP Values by Feature Value

```{r violin_plots_shap_to_feature, echo=FALSE, warning=FALSE ,message=FALSE, error=FALSE, results='asis', fig.keep='all'}
# Calculate quantiles
quantiles <- card_individual %>%
  pull({{ target_feature_shap }}) %>%
  quantile(c(0.025, 0.975), na.rm = TRUE)

# Filter data and limit categorical variable values to 30 characters
card_individual %>%
  filter(!!sym(target_feature_shap) >= quantiles[1] & !!sym(target_feature_shap) <= quantiles[2]) %>%
  mutate(!!sym(target_feature_value) := substr(!!sym(target_feature_value), 1, 25)) %>%
  select(meta_card_num, meta_pin, {{ target_feature_shap }}, {{ target_feature_value }}) %>%
  ggplot(aes(x = !!sym(target_feature_value), y = !!sym(target_feature_shap))) +
...
```{r SHAP_scatterplot, fig.keep='all'}
shapviz::shapviz(
  object = shap_new %>%
