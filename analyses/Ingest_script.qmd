model_fetch_run_subset <- function(run_id, year, paths, append_run_id = FALSE) {
  ...
  for (analyses_path in analyses_paths$output) {
    ...
    if (is_directory) {
      partitioned_by_run <- endsWith(analyses_path$s3,
      paste0("run_id=", run_id, "/"))
      if (partitioned_by_run) {
        dir_path <- analyses_path$s3
      } else {
        dir_path <- paste0(analyses_path$s3, "year=",
        year, "/run_id=", run_id, "/")
      }
      ...
      if (nrow(objs) > 0) {
        ...
        output_key <- analyses_path$key
        if (append_run_id) {
          output_key <- paste0(output_key, "_", run_id)
        }

        data_list[[output_key]] <- combined_data
      } else {
      ...
      if (aws.s3::object_exists(analyses_path$s3, bucket = bucket)) {
        ...
        aws.s3::save_object(analyses_path$s3, bucket = bucket, 
        ...
        data_list[[output_key]] <- temp_data
      } else {
        warning(analyses_path$key, " does not exist for this run")
      }
    }
