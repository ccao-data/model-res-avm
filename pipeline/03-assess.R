...
rsf_columns <- gsub(
  "_tot", "", get_rs_col_name(model_assessment_data_year, rsf_year, rsf_stage)
)
rsn_columns <- gsub(
  "_tot", "",
  get_rs_col_name(model_assessment_data_year, rsn_year, rsn_stage)
)
...
# Load the list of predictors from the metadata file
# Load the run type from the metadata file, if it exists
if (file.exists(paths$output$metadata$local)) {
  model_predictors <- read_parquet(
    paths$output$metadata$local, "model_predictor_name"
  ) %>%
    dplyr::pull(model_predictor_name) %>%
    unlist()
} else {
...
if (model_run_type %in% c("candidate", "final")) {
  ...
  assessment_data_merged %>%
    select(
      ...
      meta_complex_id, loc_longitude, loc_latitude, loc_latitude,
      pred_card_initial_fmv, pred_card_final_fmv,
      all_of(model_predictors), township_code
    ) %>%
    ccao::vars_recode(
