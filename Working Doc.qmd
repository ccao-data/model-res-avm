...
filtered_plot <- working_data_non_zero %>%
  ...
  distinct(loc_census_acs5_tract_geoid, .keep_all = TRUE) %>%
  inner_join(nbhd, by = c("loc_census_acs5_tract_geoid" = "town_nbhd")) %>%
  st_as_sf()
...
```
# Try to find out if there are properties / areas where this feature has an impact.
...
filtered_data %>%
  ...
  addCircleMarkers(
    ...
    lat = ~loc_latitude,
    radius = 5,
    color = "NA", # Border color set to black or any preferred color
...
inner_join(old_model, new_model, by = "geography_id") %>%
  ...
  kable(
    format = "html", # You can change this to "latex" or "markdown" if needed
